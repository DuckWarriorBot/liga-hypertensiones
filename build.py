...
<div class="tab-panel active" id="tab-clasificacion">
  ...
  <!-- ===== Selector de rango de jornadas para la clasificación ===== -->
  <div class="standings-round-nav" id="standingsRangeBar" style="padding:8px 0 10px;border-top:1px solid var(--border)">
    <span style="font-size:10px;color:var(--muted);font-weight:600;text-transform:uppercase;letter-spacing:.5px;margin-right:2px">Rango:</span>
    <span style="font-size:12px;color:var(--muted)">Desde J</span>
    <input type="number" id="standFromInput" min="1" max="42" value="1"
      onchange="applyStandingsRange()"
      style="width:50px;background:var(--card);border:1px solid var(--border);color:var(--text);border-radius:4px;padding:3px 6px;font-size:13px;text-align:center">
    <span style="font-size:12px;color:var(--muted)">Hasta J</span>
    <input type="number" id="standToInput" min="1" max="42" value="{total_rounds}"
      onchange="applyStandingsRange()"
      style="width:50px;background:var(--card);border:1px solid var(--border);color:var(--text);border-radius:4px;padding:3px 6px;font-size:13px;text-align:center">
    <button class="hist-sort-btn active" id="srnAll" onclick="clearStandingsRange()">Completa</button>
    <button class="hist-sort-btn" id="srnV1" onclick="setStandingsRange(1,21)">1ª Vuelta</button>
    <button class="hist-sort-btn" id="srnV2" onclick="setStandingsRange(22,{total_rounds})">2ª Vuelta</button>
    <span id="standRangeLabel" style="font-size:11px;color:var(--accent)"></span>
  </div>
  ...
  <div class="card" id="miniLigaCard" style="margin-top:4px">
    ...
    <div id="miniLigaContent" style="display:none;margin-top:14px">
      ...
      <!-- Accesos rápidos equipos -->
      <div style="font-size:10px;color:var(--muted);font-weight:600;text-transform:uppercase;letter-spacing:.5px;margin-bottom:5px">Equipos</div>
      ...
      <!-- Rango de jornadas -->
      <div style="font-size:10px;color:var(--muted);font-weight:600;text-transform:uppercase;letter-spacing:.5px;margin-bottom:5px">Rango de jornadas</div>
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:14px;padding:10px 12px;background:rgba(255,255,255,0.03);border-radius:8px;border:1px solid var(--border)">
        <div style="display:flex;align-items:center;gap:6px">
          <span style="font-size:12px;color:var(--muted)">Desde J</span>
          <input type="number" id="miniLigaFromInput" min="1" max="42" value="1"
            onchange="applyMiniLigaRange()"
            style="width:52px;background:var(--card);border:1px solid var(--border);color:var(--text);border-radius:4px;padding:4px 6px;font-size:13px;text-align:center">
          <span style="font-size:12px;color:var(--muted)">Hasta J</span>
          <input type="number" id="miniLigaToInput" min="1" max="42" value="{total_rounds}"
            onchange="applyMiniLigaRange()"
            style="width:52px;background:var(--card);border:1px solid var(--border);color:var(--text);border-radius:4px;padding:4px 6px;font-size:13px;text-align:center">
        </div>
        <div style="display:flex;gap:5px;flex-wrap:wrap">
          <button class="hist-sort-btn active" id="mlr-all" onclick="setMiniLigaRange(1,{total_rounds})">Completa</button>
          <button class="hist-sort-btn" id="mlr-v1" onclick="setMiniLigaRange(1,21)">1ª Vuelta</button>
          <button class="hist-sort-btn" id="mlr-v2" onclick="setMiniLigaRange(22,{total_rounds})">2ª Vuelta</button>
          <button class="hist-sort-btn" id="mlr-last5" onclick="setMiniLigaLastN(5)">Últ. 5J</button>
          <button class="hist-sort-btn" id="mlr-last10" onclick="setMiniLigaLastN(10)">Últ. 10J</button>
          <button class="hist-sort-btn" id="mlr-last20" onclick="setMiniLigaLastN(20)">Últ. 20J</button>
        </div>
        <span id="miniLigaRangeLabel" style="font-size:11px;color:var(--text);font-weight:600;margin-left:2px"></span>
      </div>
      ...
      <div class="card-legend" id="miniLigaFooter">Solo se contabilizan los partidos disputados entre los equipos seleccionados dentro del rango de jornadas seleccionado · útil para analizar etapas específicas (cambio de entrenador, rachas, primeras vs últimas jornadas...)</div>
    </div>
  </div>
</div>
...
let _standingsRangeMode = false;
let _standingsFromJ = 1;
let _standingsToJ = LIGA_DATA.total_rounds || 40;
...
function computeRangeStandings(fromJ, toJ) {{
  const T = LIGA_DATA.total_rounds || 40;
  const fromIdx = Math.max(0, fromJ - 1);
  const toIdx = Math.min(T - 1, toJ - 1);
  const scMap = SCORES_DATA.scores_by_team || {{}};
  return LIGA_DATA.teams.map(name => {{
    const allRes = LIGA_DATA.results_by_team[name] || [];
    const res = allRes.slice(fromIdx, toIdx + 1);
    const wins   = res.filter(x=>x==='V').length;
    const draws  = res.filter(x=>x==='E').length;
    const losses = res.filter(x=>x==='D').length;
    const pts    = wins * 3 + draws;
    ...
    const racha = (() => {{ let c=0; for(let i=allRes.length-1;i>=0;i--){{ if(allRes[i]==='D')break; c++; }} return c; }})();
    const played = res.length;
    return {{ name, wins, draws, losses, played, pts, gf, gc, dif: gf-gc, racha, allRes,
      ppg: played > 0 ? (pts/played).toFixed(2) : '0.00',
      quedan: 0, situacion: ''
    }};
  }}).sort((a,b) => b.pts - a.pts || b.wins - a.wins || b.dif - a.dif)
...
function computeStandings() {{
  if (_standingsRangeMode) return computeRangeStandings(_standingsFromJ, _standingsToJ);
...
function changeStandingsRound(delta) {{
  _standingsRangeMode = false;
  _updateStandingsRangeUI();
...
let _miniLigaFromJ = 1;
let _miniLigaToJ = LIGA_DATA.total_rounds || 40;
...
window.toggleMiniLiga = function() {{
  ...
  if (_miniLigaOpen) {{
    _miniLigaToJ = LIGA_DATA.total_rounds || 40;
    const ti = document.getElementById('miniLigaToInput');
    if (ti) ti.value = _miniLigaToJ;
    _updateMiniLigaRangeUI();
    if (_miniLigaSelected.size === 0) selectMiniLigaTopN(6);
  }}
}};

function _updateMiniLigaRangeUI() {{
  const T = LIGA_DATA.total_rounds || 40;
  const presets = [
    {{ id: 'mlr-all',   from: 1,  to: T }},
    {{ id: 'mlr-v1',   from: 1,  to: 21 }},
    {{ id: 'mlr-v2',   from: 22, to: T }},
  ];
  presets.forEach(p => {{
    const el = document.getElementById(p.id);
    if (el) el.classList.toggle('active', _miniLigaFromJ === p.from && _miniLigaToJ === p.to);
  }});
  ['mlr-last5','mlr-last10','mlr-last20'].forEach(id => {{
    const el = document.getElementById(id);
    if (el) el.classList.remove('active');
  }});
  const lbl = document.getElementById('miniLigaRangeLabel');
  if (lbl) lbl.textContent = `J${{_miniLigaFromJ}} — J${{_miniLigaToJ}} (${{_miniLigaToJ - _miniLigaFromJ + 1}} jornadas)`;
}}

window.setMiniLigaRange = function(from, to) {{
  const T = LIGA_DATA.total_rounds || 40;
  _miniLigaFromJ = Math.max(1, from);
  _miniLigaToJ = Math.min(to, T);
  const fi = document.getElementById('miniLigaFromInput');
  const ti = document.getElementById('miniLigaToInput');
  if (fi) fi.value = _miniLigaFromJ;
  if (ti) ti.value = _miniLigaToJ;
  _updateMiniLigaRangeUI();
  _renderMiniLiga();
}};

window.setMiniLigaLastN = function(n) {{
  const T = LIGA_DATA.total_rounds || 40;
  const from = Math.max(1, T - n + 1);
  _miniLigaFromJ = from;
  _miniLigaToJ = T;
  const fi = document.getElementById('miniLigaFromInput');
  const ti = document.getElementById('miniLigaToInput');
  if (fi) fi.value = _miniLigaFromJ;
  if (ti) ti.value = _miniLigaToJ;
  // Mark matching last-N button active
  _updateMiniLigaRangeUI();
  const map = {{ 5: 'mlr-last5', 10: 'mlr-last10', 20: 'mlr-last20' }};
  if (map[n]) {{ const el = document.getElementById(map[n]); if (el) el.classList.add('active'); }}
  _renderMiniLiga();
}};

window.applyMiniLigaRange = function() {{
  const fi = document.getElementById('miniLigaFromInput');
  const ti = document.getElementById('miniLigaToInput');
  const from = Math.max(1, parseInt(fi ? fi.value : 1) || 1);
  const to = Math.max(from, parseInt(ti ? ti.value : 40) || 40);
  setMiniLigaRange(from, to);
}};
...
function _renderMiniLiga() {{
  ...
  const rows = Array.from(_miniLigaSelected).map(team => {{
    ...
    const fromIdx = _miniLigaFromJ - 1;
    const toIdx = _miniLigaToJ - 1;
    for (let idx = 0; idx < T; idx++) {{
      if (idx < fromIdx || idx > toIdx) continue;  // fuera del rango seleccionado
      const opp = opps[idx];
...
// ===== STANDINGS RANGE MODE =====
function _updateStandingsRangeUI() {{
  const T = LIGA_DATA.total_rounds || 40;
  const on = _standingsRangeMode;
  const btnAll = document.getElementById('srnAll');
  const btnV1  = document.getElementById('srnV1');
  const btnV2  = document.getElementById('srnV2');
  const lbl    = document.getElementById('standRangeLabel');
  if (btnAll) btnAll.classList.toggle('active', !on);
  if (btnV1)  btnV1.classList.toggle('active',  on && _standingsFromJ===1  && _standingsToJ===21);
  if (btnV2)  btnV2.classList.toggle('active',  on && _standingsFromJ===22 && _standingsToJ===T);
  if (lbl) lbl.textContent = on ? `\u25b6 J${{_standingsFromJ}}\u2013J${{_standingsToJ}} (${{_standingsToJ - _standingsFromJ + 1}} jornadas)` : '';
}}
window.clearStandingsRange = function() {{
  _standingsRangeMode = false;
  sortCol = 'pos'; sortAsc = true;
  _updateStandingsRangeUI();
  renderStandings();
}};
window.setStandingsRange = function(f, t) {{
  _standingsFromJ = f; _standingsToJ = t;
  const fi = document.getElementById('standFromInput');
  const ti = document.getElementById('standToInput');
  if (fi) fi.value = f;
  if (ti) ti.value = t;
  _standingsRangeMode = true;
  sortCol = 'pos'; sortAsc = true;
  _updateStandingsRangeUI();
  renderStandings();
}};
window.applyStandingsRange = function() {{
  const fi = document.getElementById('standFromInput');
  const ti = document.getElementById('standToInput');
  let f = parseInt(fi?.value) || 1;
  let t = parseInt(ti?.value) || (LIGA_DATA.total_rounds || 40);
  const T = LIGA_DATA.total_rounds || 40;
  f = Math.max(1, Math.min(f, T));
  t = Math.max(f, Math.min(t, T));
  if (fi) fi.value = f;
  if (ti) ti.value = t;
  _standingsFromJ = f; _standingsToJ = t;
  _standingsRangeMode = (f > 1 || t < T);
  sortCol = 'pos'; sortAsc = true;
  _updateStandingsRangeUI();
  renderStandings();
}};
